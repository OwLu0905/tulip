...
import { TAG_TEMPLATE } from "./utils/constants.js";
 ...
 */
async function createTemplate(title, tagName) {
	...
	try {
		...
		await writeContents(templateMdxDir, mdxsDir, mdxTargetName, title, idx, tagName);
	} catch (error) {
...
async function writeContents(dir, target, targetName, title, slug, tagName = "") {
	// NOTE: read contents
	...
	const writePromise = tempFiles.map((filename, index) => {
		...
		const replacedTag = tagName ? `${TAG_TEMPLATE}: '${tagName}'` : ""
		content = content.replaceAll("{{tag}}", replacedTag);
